    ...
    //new pdf being created for current position
        ...
        if (req.file) {
            ...
            req.file.originalname
                ...
                : (pdfName = params.title + '_' + params.company + '.pdf');
        ...
        //create pdf based on new content
            ...
            var pdfName =
                newCareerContent.title +
                '_' +
                newCareerContent.company +
                '.pdf';
            ...
            //create pdf title
            doc.fontSize(24)
                .text(newCareerContent.company + ',' + newCareerContent.title)
                .moveDown();
            doc.fontSize(12);
            ...
            pdfHeaders.forEach((header) => {
                switch (header) {
                    case 'Company':
                        currentPdfText =
                            header + ': ' + newCareerContent.company;
                    ...
                    case 'Position':
                        currentPdfText =
                            header + ': ' + newCareerContent.position;
                        doc.text(currentPdfText);
                        doc.moveDown(2);
                        break;
                    case 'City':
                    ...
                    case 'Description':
                        currentPdfText =
                            header + ': ' + newCareerContent.description;
                        doc.text(currentPdfText);
                        doc.moveDown(2);
                        break;
                    default:
...
router.delete('/delete', (req, res) => {
    ...
    Positions.findOneAndUpdate(
        { _id: id },
        { $set: { deleted: true, deletedMessage: deletedMessage } },
        (err, result) => {
            if (err) {
                console.log(err);
                return res.status(500).json({
